(function() {
  var ROOT, benchmark, compileTemplate, copyFile, copyFiles, crypto, decorate, errMsg, fs, getCss, getFont, getImg, getJs, getTemplate, highlight, hljs, jade, less, markdownIt, modifyUriTemplate, moment, path, querystring, renderExample, renderSchema, schemaSubStructures, sha1, slug;
  ...
  schemaSubStructures = function(schema) {
    var item, name, properties, ref, structures, sub, subItem, subName;
    structures = {};
    if (schema.properties != null) {
      properties = schema.properties;
    } else if (((ref = schema.items) != null ? ref.properties : void 0) != null) {
      properties = schema.items.properties;
    }
    if (properties != null) {
      for (name in properties) {
        item = properties[name];
        if ((item.itemType != null) && (item.itemType !== 'string' && item.itemType !== 'number' && item.itemType !== 'object' && item.itemType !== 'enum' && item.itemType !== 'option')) {
