(function() {
  var ROOT, benchmark, compileTemplate, crypto, decorate, errMsg, fs, getCss, getFont, getTemplate, highlight, hljs, jade, less, markdownIt, modifyUriTemplate, moment, path, querystring, renderExample, renderSchema, sha1, slug;
  ...
  getFont = function(dest, verbose, done) {
    ...
    try {
      return fs.recurse(from, ['*wesome.*'], function(filepath, relative, filename) {
        return console.log(path.join(filepath, filename));
      });
    } catch (error) {
  ...
  getTemplate = function(name, verbose, done) {
    var builtin, compileErr, compileOptions, compiled, compiledPath, error, error1, error2, load, writeErr;
